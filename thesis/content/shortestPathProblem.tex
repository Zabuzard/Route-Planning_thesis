...
\chapter{Shortest path problem}\label{shortestPathProblem}
	For route planning, routes through a network must be optimized with respect to one or even many criteria.
	A common criterion is \textit{travel time}. Others include cost, number of transfers or restrictions
	...
	\begin{mydef}
		The earliest arrival problem asks for finding a \textnormal{route} in a network with the following properties.
		\begin{itemize}
...
\section{Time-independent}
	Route planning in time-independent networks is a well understood problem.
	...
	\end{figure}\quad\\
	The network shown in \figref{timeIndependentExample} acts as toy example for this section.
...
\subsection{Dijkstra}
	...
	To familiarize with the algorithm, we step through the execution for the graph shown in \figref{timeIndependentExample},
	...
	Thus, $\infty$ is used for them. $Q$ represents the list of nodes that need to be processed, usually implemented as a priority queue.
...
\subsection{\astar and \alt}\label{alt}
	...
	where the importance of nodes heavily differs, such as public transit networks. For train networks, the typical case is that one first needs
	...
	The \textit{landmark heuristic} partially solves the issue. An \astar algorithm using the landmark heuristic is called \alt \libref{alt},
	...
	between nodes $u$ and $v$ by using precomputed distances with predetermined nodes $l$, called \textit{landmarks}.
	...
	\textbf{lines 9} and \textbf{10}, such that the algorithm relaxes the whole network. By that, a single run of \dijkstra with a landmark $l$ as the source,
...
%Time-dependent
\section{Time-dependent}\label{time_dependent_sec}
	...
	\dijkstra and its variants \astar and \alt can easily be adapted to also work with time-dependent networks by taking the departure
	time into consideration when computing the weight of an edge. However, their effectiveness is very limited.
	Nonetheless, they were used for a long time for time-dependent networks too. With increasing research on route
...
%Connection scan
\subsection{Connection scan}\label{csa}
	...
	the arrival stop of $c_{\exit}$. Such an entry is associated with the arrival stop of the footpath $f$, always representing the parent path
	...
	is typically implemented as a \textit{binary search} \libref{binarySearch} on a sorted array of connections $C$.
	
	\textbf{Line 9} is the stopping criterion, which lets the algorithm terminate once a connection departs after the current best arrival
	...
	\end{align*}
	However, it induces no changes, as the previous connection already arrived in \offenburg earlier.
...
%Multi-modal
\section{Multi-modal}\label{multiModal_sec}
	So far, all presented route planning algorithms are limited to networks only consisting of routes of one transportation mode,
	...
	all footpaths \foot.\\\\
	Another difficulty of \multiModal routing is that, in practice, it is usually not applicable to change transportation modes arbitrarily.
...
%Modified Dijkstra
\subsection{Modified Dijkstra}\label{modifiedDijkstra}
	...
	\begin{figure}[!ht]
		\begin{center}
			\begin{tikzpicture}[y = -1cm]
			 	...
			 	\draw[thick, ->] (q1) to [loop above] node[above] {$m' \in M$} (q1);
			\end{tikzpicture}
		\end{center}
	...
	\end{figure}
	The modified \dijkstra accepts the transportation mode model shown in \figref{modifiedDijkstraModesAutomaton}.\\\\
	...
	the shortest path for $S = M$ can also be taken with the actual restricted version of $S$. It gets worse if some edges are
...
%Access nodes
\subsection{Access nodes}\label{accessNodes}
	...
	\begin{align*}
		...
		\dest(p_2)	&= \src(p_3).
	\end{align*}
	Of all paths satisfying these constraints, $p$ is chosen as the path with the smallest cost. Additionally, we consider the shortest path $q$
	between the source and destination that only uses the road network. The final path is again the one with the smallest cost.
	...
	\end{figure}\quad\\
	The accepted transportation mode model is shown in \figref{accessNodesModesAutomaton}.
